...
from .serializers import SessionCreateSerializer, ConversationSerializer, IngestSerializer
import jwt, os, uuid
...
class TokenExchangeView(APIView):
    ...
    def post(self, request):
        ...
        token = jwt.encode(payload, os.getenv('SECRET_KEY', 'replace-me'), algorithm='HS256')
...
class ChatMessageView(APIView):
    ...
    def post(self, request):
        ...
        if not session_id or not message:
            return Response({'detail': 'session_id and message required'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            session = Session.objects.get(id=session_id)
        except Session.DoesNotExist:
            return Response({'detail': 'session not found'}, status=status.HTTP_404_NOT_FOUND)
        # Simple echo / placeholder LLM call
        conv = Conversation.objects.create(session=session, client=session.client, user_message=message, bot_reply=f"Echo: {message}")
        return Response({'reply': conv.bot_reply, 'sources': [], 'ticket_created': False})
